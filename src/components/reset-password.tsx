...
import { z } from "zod";
import { reset } from "~/app/passwords/reset/actions";
...
export default function ResetPassword() {
  ...
  const onSubmit = handleSubmit(async ({ newPassword, repeat }) => {
    setIsLoading(true);
    await reset({
      email,
      newPassword,
    ...
    setIsLoading(false);
    router.push("https://vigbo.ru/login");
  });
  ...
  return (
    <form
      ...
      onSubmit={onSubmit}
    >
      <h1 className="text-[36px] lg:text-[44px] font-semibold">
        Изменение пароля
      </h1>
